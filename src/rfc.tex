...
\noindent The key idea is to replace \a{TC} in the offloading to match-action pipeline,
...
is supported by recent work in upstream to allow sharing of \a{TC} classifier
blocks between \netdev s, which works around a design limitation of \a{TC}, because
...
When a \emph{table} is created, the set of \emph{header fields} it uses is
...
\begin{figure}[h]
	\begin{tikzpicture}[y=0.7cm]
		...
		\node at (1.5, 2.5) {\dots};
		\node at (4.5, 2.5) {Header fields};
		\node at (6.5, 2.5) {\dots};
		\node at (1.5, 1.5) {\dots};
		\node at (4.5, 1.5) {Flows};
		\node at (6.5, 1.5) {\dots};
		\node at (1.5, .5) {\dots};
		\node at (4.5, .5) {\dots};
		\node at (6.5, .5) {\dots};
	...
	\end{tikzpicture}
	\centering
	\caption{The entities in a table.}
\end{figure}
...
technologies (\a{XDP}, \a{BPF}, custom firmware, ...).

To support more advanced features of modern \a{NIC}, we generally prefer adding
...
call \a{NAPI} to deliver the packet to the networking stack. In the other case,
...
The model we propose is similar to the \a{BPF} classifier, which is already present
...
\iltodo{Review the rest of this section below.}
...
tools for programming the hardware pipelines. The \a{P4} language serves exactly
...
\a{TC} is ideal. \a{XDP} serves those purposes much better. However, this holds only for
static scenarios -- an insertion of a rule into a \a{BPF} program means that the
...
even better to employ a kernel-bypassing solution like \a{DPDK}.
