...
The code still uses some headers from Linux, we do not expect it to be
compilable on other platforms. Due to the targeted audience, we do not expect
this to be a problem.

\section{Usage}

The source code is written in the C language and follows the GNU99 standard.
No libraries are required to compile it.

The archive contains a \texttt{Makefile} that should compile all the sources by
running \texttt{make}. If the compilation was successfull, you can run any of
...
between individual steps. As the subsystem produces debug messages to the
...
\begin{description}
	\item[\texttt{exact}, \texttt{hash}, and \texttt{tcam}] \hfill \\
		...
		the tables are configured and tests the software implementation of the
		subsystem.
...
\begin{figure}
	...
	\end{tikzpicture}
	\caption{The modules of our implementation. Highlighted is the scope of the
...
\begin{description}
	...
	\item[Netlink method calling] \hfill \\
		...
		netlink interface is designed to be very flexible and binary
		compatible, but not particularly readable.
\end{description}
...
The \texttt{netdev} and \texttt{tc} are simple modules serve as brokers to
...
supporting the stability of the \a{kABI}.
...
\struct|mat_header_field|. Both are identified by globally unique indices.
...
For every protocol defined, an instance of \struct|mat_parser| is created. The
...
\a{MAC} header directly if no \a{VLAN} tags are present. As a header field can
...
\begin{figure}
	...
	\end{tikzpicture}
	\centering
	\caption{Parsing a \a{VLAN}-tagged packet with the preconfigured parser.
...
used to extract fields. The values of well-known fields are already be extracted
by the kernel flow dissector and thus present in the \skb. However, the
definition needs to be precise not to break offload to programmable parsers.
...
its size and interpretation of contents is dependent on the table, for which the
flow key is created. Therefore, the flow key is always associated with
a concrete table, even though it does not carry a pointer to the table.
...
offloaded. The driver can either precompute the \a{TTL} to make the executor
