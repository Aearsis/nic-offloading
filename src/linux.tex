...
because they do not correspond to offloads but rather to general-purpose flags,
...
\begin{description}
	...
	\item[\macrofmt{NETIF\_F\_FCOE\_CRC} and \macrofmt{NETIF\_F\_SCTP\_CRC}]
		indicates the ability to compute \a{CRC} for \a{FCoE} and \a{SCTP} packets,
		respectively.
...
Unless handling mixed traffic which is only partially supported by \a{RSS} or
...
instruction set is used -- \acrlong{BPF}. \a{BPF} is designed to be safe to be
...
As a side note, \a{BPF} programs can be used as filters in the \a{TC}
...
The program executed in \a{XDP} can decide what to do with the packet. The
...
we can expect more controllers to support running \a{BPF} programs in the future. The
NFP is able to offload both \a{TC} \texttt{bpf} filters and \a{XDP} programs.
...
\begin{figure}
	...
	\end{tikzpicture}
	\centering
	\caption{The composition of \qdisc{}s. Filter block attached to the
...
\a{IPv4} and \a{IPv6} are not shared and must be instantiated twice if needed.
...
At first, \a{TC} offload was limited to offloading of transmit priority scheduling
