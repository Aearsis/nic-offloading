...
Unfortunately, the \a{IPv4} ID will be the same for all segments.
...
mention the \acrfull{XDP}. The idea of \a{XDP} is to allow the user to inject an
...
usually considered a security flaw. In the case of \a{XDP}, a special restricted
...
tables, and tries was added. Actually, the instruction set no longer resembles the original \a{BPF},
...
The \a{XDP} programs are invoked as soon as a packet is received by the driver,
before the \skb{} structure is allocated. That means that \a{XDP} needs to be
...
two filter blocks can be attached. One of them is used as the ingress chain as
